...
// Default file paths
const DEFAULT_DATA_FILE = (0, path_1.resolve)(process.cwd(), '../cowork_outputs/dog_images_2022_2025.json');
const DEFAULT_MAPPING_FILE = (0, path_1.resolve)(process.cwd(), '../cowork_outputs/rmgdri_rescue_master_id_mapping.csv');
// =========================================
...
function parseArgs() {
    ...
    const options = {
        ...
        mappingFile: (0, fs_1.existsSync)(DEFAULT_MAPPING_FILE) ? DEFAULT_MAPPING_FILE : null,
    };
...
// ID MAPPING
// =========================================
function loadIDMapping(mappingFile) {
    const idMap = new Map();
    if (!mappingFile || !(0, fs_1.existsSync)(mappingFile)) {
        console.log('⚠️  ID mapping file not found (this is OK - IDs will be null)');
        return idMap;
    }
    try {
        const csvContent = (0, fs_1.readFileSync)(mappingFile, 'utf-8');
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            if (wpPostId)
                idMap.set(`wp_${wpPostId}`, idNumber);
            if (slug)
                idMap.set(`slug_${slug}`, idNumber);
            if (dogName)
                idMap.set(`name_${dogName}`, idNumber);
        }
...
// DOG IMPORT
// =========================================
async function importDog(dog, idMap, options, report) {
    try {
        // Match ID number
...
// DELETE PREFIX
// =========================================
async function deleteByPrefix(prefix, dryRun, confirm) {
    if (!confirm && !dryRun) {
...
async function main() {
    ...
    if (options.deletePrefix) {
        const deletedCount = await deleteByPrefix(options.deletePrefix, options.dryRun, options.confirm);
        process.exit(0);
    }
    // Load data file
    console.log(`📂 Loading data from: ${options.dataFile}`);
    if (!(0, fs_1.existsSync)(options.dataFile)) {
        console.error(`❌ Data file not found: ${options.dataFile}`);
        process.exit(1);
    }
    const coworkData = JSON.parse((0, fs_1.readFileSync)(options.dataFile, 'utf-8'));
    ...
    // Write reports
    const reportDir = (0, path_1.join)(process.cwd(), 'scripts/rmgdri');
    const jsonReportPath = (0, path_1.join)(reportDir, 'import_run_report.json');
    const mdReportPath = (0, path_1.join)(reportDir, 'import_run_report.md');
    (0, fs_1.writeFileSync)(jsonReportPath, JSON.stringify(report, null, 2));
    console.log(`\n📄 JSON report saved: ${jsonReportPath}`);
    const mdReport = generateMarkdownReport(report);
    (0, fs_1.writeFileSync)(mdReportPath, mdReport);
