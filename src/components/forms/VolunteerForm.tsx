...
function FieldInput({
  ...
  /* ── roles (special canonical checkbox list) ── */
  if (field.type === "roles") {
        ...
        {VOLUNTEER_ROLES.map((role) => (
          ...
          >
            <input
              ...
              onChange={() => {
                const next = selected.includes(role.id)
                  ? selected.filter((v) => v !== role.id)
                  : [...selected, role.id];
...
export default function VolunteerForm() {
  ...
  const handleChange = useCallback((key: string, val: string | string[] | boolean) => {
  ...
  const handleSubmit = useCallback(
    async (e: FormEvent) => {
      ...
      try {
        const res = await fetch("/api/forms/volunteer/submit", {
          method: "POST",
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-10" noValidate>
      ...
      {VOLUNTEER_SECTIONS.map((section) => {
        const fields = VOLUNTEER_FIELD_MAP.filter((f) => f.section === section);
        return (
          <fieldset key={section} className="space-y-5">
            <legend className="text-xl font-semibold text-gray-900 border-b border-gray-200 pb-2 mb-4">
              {section}
            </legend>
