...
export async function POST(req: NextRequest) {
    ...
    return NextResponse.json(
      {
        ...
        message:
          "Photo uploads are not yet configured. Please email your photos to shelterrequests@rmgreatdane.org.",
      },
      { status: 503 }
    );
  }
  ...
  try {
    const key = generateKey("rmgdri-media/intake/shelter-transfer", file.name);
    ...
    console.error("[shelter-transfer/upload] R2 error:", e?.message ?? e);
    return NextResponse.json(
      { error: "upload_failed", message: "Failed to upload photo. Please try again or email your photos to shelterrequests@rmgreatdane.org." },
      { status: 500 }
    );
  }
}
